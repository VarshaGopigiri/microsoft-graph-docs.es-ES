---
title: valores de timeZoneStandard
description: " Valor"
localization_priority: Normal
ms.openlocfilehash: 6f9a08a7f20ff9b4f5463c6d7e4877568c49eee1
ms.sourcegitcommit: d2b3ca32602ffa76cc7925d7f4d1e2258e611ea5
ms.translationtype: MT
ms.contentlocale: es-ES
ms.lasthandoff: 01/11/2019
ms.locfileid: "27825735"
---
### <a name="timezonestandard-values"></a>valores de timeZoneStandard

| Valor
|:-----------------
| Windows
| IANA


### <a name="freebusystatus-values"></a>valores de freeBusyStatus

| Miembro	            |Valor
|:------------------|:-------
| libre              | 0
| provisional         | 1
| no disponible              | 2
| fuera de la oficina               | 3
| workingElsewhere  | 4
| desconocido           | -1


### <a name="attendeetype-values"></a>valores de attendeeType

| Valor
|:-------------------------
| necesario
| opcional
| resource


### <a name="externalaudiencescope-values"></a>valores de externalAudienceScope

| Valor
|:-------------------------
| none
| contactsOnly
| all


### <a name="automaticrepliesstatus-values"></a>valores de automaticRepliesStatus

| Valor
|:-------------------------
| deshabilitado
| alwaysEnabled
| programado


### <a name="calendarcolor-values"></a>valores de calendarColor

| Miembro	     | Valor
|:-----------|:----------
| Automático       | -1
| lightBlue  | 0
| verde claro | 1
| lightOrange| 2
| lightGray  | 3
| amarillo claro| 4
| lightTeal  | 5
| Rosa claro  | 6
| lightBrown | 7
| rojo claro   | 8
| maxColor   | 9


### <a name="educationexternalsource-values"></a>valores de educationExternalSource

| Valor
|:-------------------------
| SIS
| Manual
| unknownFutureValue


### <a name="educationgender-values"></a>valores de educationGender

| Valor
|:-------------------------
| hembra
| hombre
| otros
| unknownFutureValue


### <a name="eventtype-values"></a>valores de enumeración eventType

| Valor
|:-------------------------
| singleInstance
| repetición
| excepción
| seriesMaster


### <a name="sensitivity-values"></a>valores de sensibilidad

| Valor
|:-------------------------
| normal
| personal
| privada
| información confidencial


### <a name="importance-values"></a>valores de importancia

| Valor
|:-------------------------
| baja
| normal
| alta


### <a name="educationuserrole-values"></a>valores de educationUserRole
| Valor
|:---------------------
| student
| teacher
| none
| unknownFutureValue


### <a name="meetingmessagetype-values"></a>valores de meetingMessageType

| Valor
|:-----------------
| none
| meetingRequest
| meetingCancelled
| meetingAccepted
| meetingTentativelyAccepted
| meetingDeclined


### <a name="followupflagstatus-values"></a>valores de followupFlagStatus

| Valor
|:-------------------------
| notFlagged
| complete
| marca


### <a name="inferenceclassificationtype-values"></a>valores de inferenceClassificationType

| Valor
|:-----------------
| centrada
| otros


### <a name="iosnotificationalerttype-values"></a>valores de iosNotificationAlertType

| Valor
|:-------------------------
| deviceDefault
| Banner
| modal
| none

### <a name="deviceenrollmentfailurereason-values"></a>valores de deviceEnrollmentFailureReason

| Valor
|:-------------
| desconocido
| autenticación
| autorización
| accountValidation
| userValidation
| deviceNotSupported
| inMaintenance
| badRequest
| featureNotSupported
| enrollmentRestrictionsEnforced
| clientDisconnected


### <a name="bodytype-values"></a>valores de bodyType
| Valor
|:---------
| text
| html


### <a name="locationtype-values"></a>locationType valores

| Valor
|:-------------------------
| Es el valor predeterminado.
| conferenceRoom
| homeAddress
| businessAddress
| coordenadas geográficas
| streetAddress
| hotel
| restaurante
| localBusiness
| postalAddress

### <a name="locationuniqueidtype-values"></a>valores de locationUniqueIdType

| Valor
|:-------------------------
| desconocido
| locationStore
| Active Directory
| privada
| Bing


### <a name="messageactionflag-values"></a>valores de messageActionFlag

| Valor
|:-------------------------
| cualquiera
| call
| doNotForward
| realizar su seguimiento
| para su información
| desviar
| noResponseNecessary
| lectura
| responder
| replyToAll
| revisión


### <a name="onenoteuserrole-values"></a>valores de onenoteUserRole

| Miembro	      | Valor
|:------------|:------------
| Owner       | 0
| Colaborador | 1
| Lector      | 2
| Ninguno        | -1


### <a name="operationstatus-values"></a>valores de operationStatus

| Valor
|:-----------------
| No iniciado
| En ejecución
| Completed
| Failed


### <a name="onenotepatchactiontype-values"></a>valores de onenotePatchActionType

| Valor
|:-------------------------
| Reemplazar
| Anexar
| Delete
| Insertar
| Anteponga

### <a name="onenotepatchinsertposition-values"></a>valores de onenotePatchInsertPosition

| Valor
|:-------------------------
| After
| Before


### <a name="phonetype-values"></a>valores de phoneType

| Valor
|:-------------------------
| página principal
| empresarial
| móvil
| otros
| Ayudante
| Fax particular
| businessFax
| otherFax
| Localizador
| radio


### <a name="plannerpreviewtype-values"></a>valores de plannerPreviewType

| Valor
|:-------------------------
| Automático
| noPreview
| checklist
| descripción
| reference


### <a name="status-values"></a>valores de estado

| Valor
|:-----------------
| activo
| se actualizó
| deleted
| pasa por alto
| unknownFutureValue


### <a name="weekindex-values"></a>valores de weekIndex

| Valor
|:-------------------------
| primera
| segundo
| tercer
| cuarto
| última


### <a name="dayofweek-values"></a>valores de dayOfWeek

| Valor
|:-------------------------
| domingo
| lunes
| Martes
| Miércoles
| Jueves
| viernes
| sábado

### <a name="recurrencepatterntype-values"></a>valores de recurrencePatternType

| Valor
|:-------------------------
| cada día
| semanal
| absoluteMonthly
| relativeMonthly
| absoluteYearly
| relativeYearly


### <a name="recurrencerangetype-values"></a>valores de recurrenceRangeType

| Valor
|:-------------------------
| endDate
| noEnd
| numeradas


### <a name="onenotesourceservice-values"></a>valores de onenoteSourceService
| Valor
|:---------------------
| Desconocido
| OneDrive
| OneDriveForBusiness
| OnPremOneDriveForBusiness


### <a name="responsetype-values"></a>valores de responseType

| Valor
|:-------------------------
| none
| organizador
| tentativelyAccepted
| aceptado
| rechazado
| notResponded


### <a name="activitydomain-values"></a>valores de activityDomain

| Valor
|:-------------------------
| desconocido
| trabajo
| personal
| sin restricciones


### <a name="websitetype-values"></a>valores de websiteType

| Valor
|:-------------------------
| otros
| página principal
| trabajo
| blog
| perfil


### <a name="categorycolor-values"></a>valores de categoryColor

| Miembro	   |Valor    
|:---------|:--------
| none     | -1      
| preset0  | 0       
| preset1  | 1       
| preset2  | 2       
| preset3  | 3       
| preset4  | 4       
| preset5  | 5       
| preset6  | 6       
| preset7  | 7       
| preset8  | 8       
| preset9  | 9       
| preset10 | 10      
| preset11 | 11      
| preset12 | 12      
| preset13 | 13      
| preset14 | 14      
| preset15 | 15      
| preset16 | 16      
| preset17 | 17      
| preset18 | 18      
| preset19 | 19      
| preset20 | 20      
| preset21 | 21      
| preset22 | 22      
| preset23 | 23      
| preset24 | 24      

# <a name="security-api-enums"></a>Enumeraciones de la API de seguridad

# <a name="alertfeedback-enum-type"></a>tipo de enumeración alertFeedback

Valores posibles comentarios en la alerta proporcionado por un analista.

## <a name="members"></a>Miembros

|Miembro	|Valor|Description|
|:---|:---|:---|
|desconocido|0|Unknown (desconocido).|
|truePositive|1|Alerta es true positivo.|
|falsePositive|2| Alerta es falsos positivos.|
|benignPositive|3| Alerta es favorable positivo.|

# <a name="filehashtype-enum-type"></a>tipo de enumeración fileHashType

Enumeración de tipos de hash de archivo.

## <a name="members"></a>Miembros

|Miembro	|Valor|Description|
|:---|:---|:---|
|desconocido|0|Tipo desconocido.|
|SHA1|1|Tipo de hash de SHA1.|
|SHA256|2| Tipo de hash SHA256.|
|MD5|3| Tipo de hash MD5.|
|authenticodeHash256|4| Tipo de algoritmo hash AuthenticodeHash256.|
|lsHash|5| Tipo de algoritmo hash LsHash.|
|ctph|6| Tipo de algoritmo hash CTPH.|
|peSha1|7| Tipo de algoritmo hash PESHA1.|
|peSha256|8| Tipo de algoritmo hash PESHA256.|

# <a name="connectiondirection-enum-type"></a>tipo de enumeración connectionDirection

Enumeración para la dirección de la conexión de red (entrante y saliente).

## <a name="members"></a>Miembros

|Miembro	|Valor|Description|
|:---|:---|:---|
|desconocido|0|Conexión desconocida.|
|de entrada|1|Conexión de entrada.|
|saliente|2| Conexión saliente.|

# <a name="connectionstatus-enum-type"></a>tipo de enumeración connectionStatus

Enumeración para el estado de las conexiones.

## <a name="members"></a>Miembros

|Miembro	|Valor|Description|
|:---|:---|:---|
|desconocido|0|Estado de conexión desconocido.|
|ha intentado|1|Intenta establecer la conexión.|
|succeeded|2| Se ha realizado correctamente la conexión.|
|bloqueado|3| Conexión bloqueada.|
|failed|4| Error en la conexión.|

# <a name="processintegritylevel-enum-type"></a>tipo de enumeración processIntegrityLevel

Valores de nivel de la integridad de los posibles del proceso.

## <a name="members"></a>Miembros

|Miembro	|Valor|Description|
|:---|:---|:---|
|desconocido|0|Unknown (desconocido).|
|no es de confianza|10|Es el nivel de integridad Untrusted.|
|baja|20| Nivel de integridad es baja.|
|medium|30| Nivel de integridad es Medium.|
|alta|40| Nivel de integridad es alto.|
|system|50| Nivel de integridad es sistema.|

# <a name="registryhive-enum-type"></a>tipo enum registryHive

Enumeración para el registro secciones tal como se define por [https://docs.microsoft.com/en-us/windows/desktop/sysinfo/registry-hives](https://docs.microsoft.com/en-us/windows/desktop/sysinfo/registry-hives).

## <a name="members"></a>Miembros

|Miembro	|Valor|Description|
|:---|:---|:---|
|desconocido|0|Subárbol desconocido.|
|currentConfig|1|Subárbol HKEY_CURRENT_CONFIG.|
|currentUser|2| Sección HKEY_CURRENT_USER.|
|localMachineSam|3| Subárbol HKEY_LOCAL_MACHINE\SAM.|
|localMachineSamSoftware|4| Subárbol HKEY_LOCAL_MACHINE\Software.|
|localMachineSystem|5| Subárbol HKEY_LOCAL_MACHINE\SYSTEM.|
|usersDefault|6| HKEY_USERS\\. Subárbol de forma predeterminada.|

# <a name="registryoperation-enum-type"></a>tipo de enumeración registryOperation

Operación que ha cambiado el nombre de la clave del registro o valor.

## <a name="members"></a>Miembros

|Miembro	|Valor|Description|
|:---|:---|:---|
|desconocido|0|Tipo de valor del registro desconocido.|
|create|1|Crear el registro.|
|modificar|2|Modificar el registro.|
|delete|3|Eliminar del registro.|

# <a name="registryvaluetype-enum-type"></a>tipo de enumeración registryValueType

Tipos de enumeración de valor del registro tal como se define por [https://docs.microsoft.com/en-us/windows/desktop/sysinfo/registry-value-types](https://docs.microsoft.com/en-us/windows/desktop/sysinfo/registry-value-types).

## <a name="members"></a>Miembros

|Miembro	|Valor|Description|
|:---|:---|:---|
|desconocido|0|Tipo de valor del registro desconocido.|
|binario|1|Tipo de valor del Registro REG_BINARY.|
|DWORD|2| Tipo de valor REG_DWORD del registro.|
|dwordLittleEndian|3| Tipo de valor del registro REG_DWORD_LITTLE_ENDIAN.|
|dwordBigEndian|4| Tipo de valor del registro REG_DWORD_BIG_ENDIAN.|
|expandSz|5| Tipo de valor del Registro REG_EXPAND_SZ.|
|vincular|6| Tipo de valor del registro REG_LINK.|
|multiSz|7| Tipo de valor del Registro REG_MULTI_SZ.|
|none|8| Tipo de valor REG_NONE del registro.|
|QWORD|9| Tipo de valor del Registro REG_QWORD.|
|qwordlittleEndian|10| Tipo de valor del registro REG_QWORD_LITTLE_ENDIAN.|
|sz|11| Tipo de valor del Registro REG_SZ.|

# <a name="alertseverity-enum-type"></a>tipo de enumeración alertSeverity

Enumeración de gravedad de alertas.

## <a name="members"></a>Miembros

|Miembro	|Valor|Description|
|:---|:---|:---|
|desconocido|0|Gravedad es desconocida.|
|informativo|1|Gravedad es sólo para información.|
|baja|2| Gravedad es baja.|
|medium|3| Gravedad es medium.|
|alta|4| Gravedad es alta.|

# <a name="alertstatus-enum-type"></a>tipo de enumeración alertStatus

Posibles valores de un estado de ciclo de vida de alerta (escenario).

## <a name="members"></a>Miembros

|Miembro	|Valor|Description|
|:---|:---|:---|
|desconocido|0|Estado desconocido.|
|newAlert|10| Alerta es nuevo.|
|en curso|20|Alerta está en curso.|
|resolver|30|Alerta se ha resuelto.|

# <a name="emailrole-enum-type"></a>tipo de enumeración emailRole

Valores posibles para funciones de correo electrónico.

## <a name="members"></a>Miembros

|Miembro	|Valor|Description|
|:---|:---|:---|
|desconocido|0|Rol desconocido.|
|sender|1|Remitente del correo electrónico.|
|destinatario|2|Destinatario del correo electrónico.|

# <a name="logontype-enum-type"></a>tipo de enumeración logonType

Valores posibles para el método de inicio de sesión de usuario.

## <a name="members"></a>Miembros

|Miembro	|Valor|Description|
|:---|:---|:---|
|desconocido|-1|Unknown (desconocido).|
|interactiva|0|Inicio de sesión es interactivo.|
|remoteInteractive|1| Inicio de sesión es remoto interactiva.|
|red|2| Inicio de sesión es la red.|
|batch|3| Inicio de sesión es por lotes.|
|service|4| Inicio de sesión es el servicio.|

# <a name="useraccountsecuritytype-enum-type"></a>tipo de enumeración userAccountSecurityType

Valores posibles para los tipos de cuenta de usuario (pertenencia a grupos), por definición de Windows.

## <a name="members"></a>Miembros

|Miembro	|Valor|Description|
|:---|:---|:---|
|desconocido|-1|Unknown (desconocido).|
|estándar|0|Miembro del grupo de usuarios estándar.|
|Power|1| Miembro del grupo Usuarios avanzados.|
|Administrador|2| Miembro del grupo Administradores.|
