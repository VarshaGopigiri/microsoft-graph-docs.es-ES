...
|id|Cadena|Identificador único del mensaje de evento (tenga en cuenta que este valor puede cambiar si se mueve o se altera un mensaje)|
|importance|Cadena| La importancia del mensaje: `low`, `normal` y `high`.|
|inferenceClassification|Cadena| Los valores posibles son: `focused` y `other`.|
...
|meetingMessageType|meetingMessageType| El tipo de mensaje de evento: `none`, `meetingRequest`, `meetingCancelled`, `meetingAccepted`, `meetingTenativelyAccepted` y `meetingDeclined`.|
