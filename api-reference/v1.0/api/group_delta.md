...
Get recién creado, actualiza o elimina grupos, incluidos los cambios de pertenencia de grupo, sin tener que realizar un acceso completo de lectura de la colección de todo grupo. Para obtener más información, vea [Uso de consulta de Delta](../../../concepts/delta_query_overview.md) .
...
- Puede usar `$expand=members` para obtener los cambios de pertenencia.
...
Si se ejecuta correctamente, este método devuelve el código de respuesta `200 OK` y el objeto de colección [grupo](../resources/group.md) en el cuerpo de la respuesta. La respuesta incluye también un símbolo (token) de estado que puede ser un `nextLink` dirección URL o un `deltaLink` dirección URL.
...
El siguiente es un ejemplo de la respuesta al usar `deltaLink` obtenido de la inicialización de la consulta. Tenga en cuenta que `description` y `mailNickname` tienen el valor de `null` lo que significa que es posible que no se han cambiado o se han establecido en un valor vacío.
...
El siguiente es un ejemplo de la respuesta al usar `deltaLink` obtenido de la inicialización de la consulta. Tenga en cuenta que el `mailNickname` (propiedad) no se incluye, lo que significa que no ha cambiado desde la última consulta delta; `displayName` y `description` se incluyen lo que significa que se han cambiado sus valores.
...
- [Obtener los cambios incrementales de grupos](../../../concepts/delta_query_groups.md).
