...
Recupera las propiedades y las relaciones del objeto [eventMessage](../resources/eventmessage.md). Aplicar el `$expand` parámetro en la propiedad de navegación de **eventos** para obtener el asociado de [evento](../resources/event.md) en el calendario de un asistente.

### <a name="get-the-event-message-body-in-html-or-text-format"></a>Obtener el cuerpo del mensaje de evento en formato HTML o de texto

Cuerpos de mensaje de evento pueden estar en formato HTML o de texto.

Puede usar el `Prefer: outlook.body-content-type` devuelve el encabezado para especificar el formato deseado en las propiedades del **cuerpo** y **uniqueBody** en una `GET` solicitud:

- Especificar `Prefer: outlook.body-content-type="text"` para obtener un cuerpo del mensaje de evento devuelven en formato de texto.
- Especificar `Prefer: outlook.body-content-type="html"`, o simplemente omita el encabezado, para devolver el cuerpo del mensaje de evento en formato HTML.
...
Si se ejecuta correctamente, este método devuelve un código de respuesta `200 OK` y el objeto [eventMessage](../resources/eventmessage.md) en el cuerpo de la respuesta.
...
Este es un ejemplo de la respuesta. Las propiedades del evento asociado se devuelven en la respuesta.
