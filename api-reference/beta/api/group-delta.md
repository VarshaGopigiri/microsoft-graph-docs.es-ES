...
| $skiptoken | string | Un [token de estado](/graph/delta-query-overview) que se devuelve en la URL de `nextLink` de la llamada de función **delta**. Indica que debe realizarse el seguimiento de más cambios en la misma colección de grupos. |
...
- No hay compatibilidad limitada para `$filter`:
  - La única expresión `$filter` admitida es para realizar un seguimiento de los cambios en un objeto específico: `$filter=id+eq+{value}`. Puede filtrar varios objetos. Por ejemplo, `https://graph.microsoft.com/beta/groups/delta/?$filter= id eq '477e9fc6-5de7-4406-bb2a-7e5c83c9ffff' or id eq '004d6a07-fe70-4b92-add5-e6e37b8affff'`. Hay un límite de 50 objetos filtrados.
...
- [Obtener los cambios incrementales de grupos](/graph/delta-query-groups).
