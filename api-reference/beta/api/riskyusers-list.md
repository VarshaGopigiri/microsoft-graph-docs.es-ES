...
Este método es compatible con `$filter` para personalizar la respuesta de consulta. Vea el ejemplo más adelante en este tema. 
...
Si tiene éxito, este método devuelve una `200 OK` objeto de código y [identityRiskEvent](../resources/identityriskevent.md) de respuesta en el cuerpo de la respuesta.
...
En el ejemplo siguiente se muestra cómo usar `$filter` para obtener la colección de riskyUser cuyo riesgo agregado nivel es Medium.
<!-- {
  "blockType": "request",
