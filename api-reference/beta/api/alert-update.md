...
|comentarios|enumeración alertFeedback|Comentarios de analistas de la alerta. Los valores posibles son: `unknown`, `truePositive`, `falsePositive` y `benignPositive`.|
|status|enumeración alertStatus|Estado de alerta del ciclo de vida (escenario). Los valores posibles son: `unknown`, `newAlert`, `inProgress` y `resolved`.|
