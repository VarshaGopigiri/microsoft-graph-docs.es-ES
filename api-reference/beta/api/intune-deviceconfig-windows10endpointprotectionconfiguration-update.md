...
|xboxServicesEnableXboxGameSaveTask|Boolean|Esta configuración determina si guardar juego xbox está habilitado (1) o deshabilitado (0).|
...
|localSecurityOptionsBlockMicrosoftAccounts|Boolean|Impedir que los usuarios agreguen nuevas cuentas de Microsoft a este equipo.|
|localSecurityOptionsBlockRemoteLogonWithBlankPassword|Boolean|Habilite las cuentas locales que no están protegido para iniciar sesión desde ubicaciones que no sean el dispositivo físico de contraseña. Es el comportamiento predeterminado|
|localSecurityOptionsEnableAdministratorAccount|Boolean|Determina si la cuenta de administrador Local está habilitada o deshabilitada.|
...
|localSecurityOptionsEnableGuestAccount|Boolean|Determina si la cuenta de invitado está habilitada o deshabilitada.|
...
|localSecurityOptionsAllowUndockWithoutHavingToLogon|Boolean|Impedir que se desacoplado sin tener que inicie sesión un equipo portátil.|
|localSecurityOptionsBlockUsersInstallingPrinterDrivers|Boolean|Restringir instalar controladores de impresora como parte de la conexión a una impresora compartida a los administradores de únicamente.|
|localSecurityOptionsBlockRemoteOpticalDriveAccess|Boolean|Habilitación de esta configuración permite único usuario inició la sesión de forma interactiva a los medios de CD-ROM de access.|
...
|localSecurityOptionsDoNotRequireCtrlAltDel|Boolean|Requerir CTRL + ALT + SUPR que se debe presionar antes de que un usuario puede iniciar sesión.|
|localSecurityOptionsHideLastSignedInUser|Boolean|No mostrar el nombre de usuario de la última persona que ha iniciado sesión en este dispositivo.|
|localSecurityOptionsHideUsernameAtSignIn|Boolean|No mostrar el nombre de usuario de la persona que inicio de sesión para este dispositivo después de que se escriben las credenciales y antes de que se muestra el escritorio del dispositivo.|
...
|localSecurityOptionsAllowPKU2UAuthenticationRequests|Boolean|Bloque PKU2U las solicitudes de autenticación para este dispositivo para utilizar las identidades en línea.|
|localSecurityOptionsAllowRemoteCallsToSecurityAccountsManagerHelperBool|Boolean|Auxiliares de la interfaz de usuario booleano de entidad LocalSecurityOptionsAllowRemoteCallsToSecurityAccountsManager|
...
|lanManagerWorkstationEnableInsecureGuestLogons|Boolean|Si se habilita, el cliente SMB le permitirá los inicios de sesión de invitado no segura. Si no está definida, el cliente SMB rechazará los inicios de sesión de invitado no segura.|
|localSecurityOptionsClearVirtualMemoryPageFile|Boolean|Esta configuración de seguridad determina si el archivo de paginación de memoria virtual se borra cuando se apaga el sistema.|
|localSecurityOptionsAllowSystemToBeShutDownWithoutHavingToLogOn|Boolean|Esta configuración de seguridad determina si un equipo se puede apagar sin tener que volver a iniciar sesión en Windows.|
|localSecurityOptionsAllowUIAccessApplicationElevation|Boolean|Permitir aplicaciones UIAccess pedir confirmación de elevación sin usar el escritorio seguro.|
|localSecurityOptionsVirtualizeFileAndRegistryWriteFailuresToPerUserLocations|Boolean|Virtualizar archivo y registro de errores de escritura por ubicaciones de usuario|
|localSecurityOptionsOnlyElevateSignedExecutables|Boolean|Aplica la validación de ruta de acceso de certificación PKI para un determinado archivo ejecutable antes de que se le permite ejecutar.|
...
|localSecurityOptionsSwitchToSecureDesktopWhenPromptingForElevation|Boolean|Habilitar todas las solicitudes de elevación ir al escritorio del usuario interactivo en lugar del escritorio seguro. Se usa la configuración de la directiva de comportamiento de solicitud para los administradores y usuarios estándar.|
|localSecurityOptionsDetectApplicationInstallationsAndPromptForElevation|Boolean|Las instalaciones de aplicaciones que requieren privilegios elevados solicitará las credenciales de administrador. Es el comportamiento predeterminado|
|localSecurityOptionsAllowUIAccessApplicationsForSecureLocations|Boolean|Permitir aplicaciones UIAccess pedir confirmación de elevación sin usar el escritorio seguro. Es el comportamiento predeterminado|
|localSecurityOptionsUseAdminApprovalMode|Boolean|Define si la cuenta de administrador integrada usa el modo de aprobación de administrador o ejecuta todas las aplicaciones con privilegios completos de administrador. Es el comportamiento predeterminado|
|localSecurityOptionsUseAdminApprovalModeForAdministrators|Boolean|Definir si están habilitadas el modo de aprobación de administrador y todas las configuraciones de directiva UAC, es el comportamiento predeterminado|
...
|localSecurityOptionsDisableClientDigitallySignCommunicationsIfServerAgrees|Boolean|Esta configuración de seguridad determina si el cliente SMB intenta negociar la firma de paquetes SMB.|
|localSecurityOptionsClientDigitallySignCommunicationsAlways|Boolean|Esta configuración de seguridad determina si el componente de cliente SMB requiere la firma de paquetes.|
|localSecurityOptionsClientSendUnencryptedPasswordToThirdPartySMBServers|Boolean|Si se habilita esta configuración de seguridad, se permite el redirector de bloque de mensajes del servidor (SMB) para enviar contraseñas de texto simple a servidores SMB que no son Microsoft que no admiten el cifrado de contraseña durante la autenticación.|
|localSecurityOptionsDisableServerDigitallySignCommunicationsAlways|Boolean|Esta configuración de seguridad determina si el componente de servidor SMB requiere la firma de paquetes.|
|localSecurityOptionsDisableServerDigitallySignCommunicationsIfClientAgrees|Boolean|Esta configuración de seguridad determina si el servidor SMB negociará la firma con clientes que lo soliciten de paquetes SMB.|
|localSecurityOptionsRestrictAnonymousAccessToNamedPipesAndShares|Boolean|De forma predeterminada, esta configuración de seguridad restringe el acceso anónimo a recursos compartidos y canalizaciones a la configuración de canalizaciones con nombre que se pueden tener acceso de forma anónima y los recursos compartidos que se pueden tener acceso de forma anónima|
|localSecurityOptionsDoNotAllowAnonymousEnumerationOfSAMAccounts|Boolean|Esta configuración de seguridad determina qué permisos adicionales se concederán para las conexiones anónimas al equipo.|
|localSecurityOptionsAllowAnonymousEnumerationOfSAMAccountsAndShares|Boolean|Esta configuración de seguridad determina si se permite a los usuarios anónimos realizar determinadas actividades, como enumerar los nombres de las cuentas de dominio y recursos compartidos de red.|
|localSecurityOptionsDoNotStoreLANManagerHashValueOnNextPasswordChange|Boolean|Esta configuración de seguridad determina si, en el próximo cambio de contraseña, se almacena el valor de hash de LAN Manager (LM) para la nueva contraseña. No se almacena de forma predeterminada.|
...
|defenderSecurityCenterDisableAppBrowserUI|Boolean|Se usa para deshabilitar la presentación de la zona de protección de aplicación y el explorador.|
|defenderSecurityCenterDisableFamilyUI|Boolean|Se usa para deshabilitar la presentación del área de opciones de la familia.|
|defenderSecurityCenterDisableHealthUI|Boolean|Se usa para deshabilitar la presentación del área de mantenimiento y rendimiento de dispositivo.|
|defenderSecurityCenterDisableNetworkUI|Boolean|Se usa para deshabilitar la presentación de la zona de protección de firewall y de red.|
|defenderSecurityCenterDisableVirusUI|Boolean|Se usa para deshabilitar la presentación de la zona de protección contra virus y amenazas.|
|defenderSecurityCenterDisableAccountUI|Boolean|Se usa para deshabilitar la presentación de la zona de protección de la cuenta.|
|defenderSecurityCenterDisableHardwareUI|Boolean|Se usa para deshabilitar la presentación de la zona de protección de hardware.|
|defenderSecurityCenterDisableRansomwareUI|Boolean|Se usa para deshabilitar la presentación de la zona de protección ransomware. |
|defenderSecurityCenterDisableSecureBootUI|Boolean|Se usa para deshabilitar la presentación del área de inicialización segura en seguridad del dispositivo.|
|defenderSecurityCenterDisableTroubleshootingUI|Boolean|Se usa para deshabilitar la visualización del proceso de seguridad de solución de problemas en la seguridad del dispositivo.|
...
|deviceGuardEnableVirtualizationBasedSecurity|Boolean|Activa en la virtualización habían basada Security(VBS).|
|deviceGuardEnableSecureBootWithDMA|Boolean|Especifica si el nivel de seguridad de plataforma está habilitado en el siguiente reinicio.|
...
|applicationGuardAllowVirtualGPU|Boolean|Permitir protección de aplicación para usar GPU virtual|
|applicationGuardAllowFileSaveOnHost|Boolean|Permitir a los usuarios descargar archivos desde el borde en el contenedor de protección de la aplicación y guardarlos en el host de sistema de archivos|
