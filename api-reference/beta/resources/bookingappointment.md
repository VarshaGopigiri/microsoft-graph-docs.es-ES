...
|customerName|Cadena|El nombre del cliente.|
|customerNotes|Cadena|Notas de cliente asociado a esta cita. Puede obtener el valor de sólo lectura cuando este **bookingAppointment** por su identificador. <br> Puede establecer esta propiedad sólo cuando se crea inicialmente una cita con un cliente nuevo. Después de ese momento, el valor se calcula a partir del cliente representado por **CustomerID (IdDeCliente)**.|
|customerPhone|Cadena|Número de teléfono del cliente.|
...
|id|Cadena| El identificador de la **bookingAppointment**. Solo lectura.|
...
|invoiceId|Cadena|El identificador de la factura.|
|invoiceStatus|string| El estado de la factura. Los valores posibles son: `draft`, `reviewing`, `open`, `canceled`, `paid`, `corrective`.|
|invoiceUrl|Cadena|La dirección URL de la factura en Microsoft Bookings.|
...
|selfServiceAppointmentId|Cadena|Un identificador de seguimiento adicional para la cita, si la cita se ha creado directamente por el cliente en la página programación, en lugar de hacerlo por un miembro del personal en el nombre del cliente.|
|serviceId|Cadena|El identificador de la [bookingService](bookingservice.md) asociadas con esta cita.|
|serviceLocation|[location](location.md)|La ubicación donde se entrega el servicio.|
|serviceName|Cadena|El nombre de la **bookingService** asociado a esta cita.<br>Esta propiedad es opcional cuando se crea una nueva cita. Si no se especifica, se calcula desde el servicio asociado con la cita por la propiedad **serviceId** .|
|serviceNotes|Cadena|Notas de un [bookingStaffMember](bookingstaffmember.md). El valor de esta propiedad está disponible sólo cuando se lee este **bookingAppointment** por su identificador.|
