...
Para obtener una lista completa de los permisos de Microsoft Graph, así como las diferencias entre los permisos delegados y de la aplicación, vea la [Referencia de permisos](permissions_reference.md).
...
- Para conocer los pasos rápidos para conseguir un token de acceso para aplicaciones que llaman a Microsoft Graph en nombre de un usuario, vea [Get access on behalf of users](auth_v2_user.md) (Obtener acceso en nombre de los usuarios).
- Para conocer los pasos rápidos para conseguir un token de acceso para aplicaciones que llaman a Microsoft Graph sin un usuario, vea [Get access without a user](auth_v2_user.md) (Obtener acceso sin un usuario).
- Para ver los permisos que se pueden usar con Microsoft Graph, vea la referencia sobre los [permisos](permissions_reference.md).
- Si es un Proveedor de soluciones en la nube de Microsoft y le interesa tener acceso a los datos de clientes administrados por el asociado a través de Microsoft Graph, vea [Manage app access (CSPs)](auth_cloudsolutionprovider.md) (Administrar el acceso de la aplicación (CSP)).
...
### <a name="microsoft-graph-connect-samples"></a>Ejemplo de Connect de Microsoft Graph 

Microsoft publica ejemplos de Connect para Microsoft Graph para una amplia variedad de plataformas, incluidas las siguientes: Android, Angular.JS, ASP.NET, iOS (Obj-C y Swift), Node.JS, PHP, Python, Ruby, UWP y Xamarin. Puede usar estos ejemplos para examinar el código que usa varias bibliotecas de autenticación para obtener tokens de Azure AD. Actualmente, la mayoría de los ejemplos usa bibliotecas de autenticación de terceros, pero los ejemplos de ASP.NET y UWP usan bibliotecas de Microsoft.

- En la sección [Compilar una aplicación por primera vez](get-started.md) se incluyen artículos detallados en los que se muestra cómo crear aplicaciones de Connect mediante el punto de conexión de Azure AD v2.0 y se incluyen las bibliotecas de autenticación de cada plataforma. Los ejemplos disponibles se enumeran por plataforma y punto de conexión de autenticación.
- Para obtener rápidamente un ejemplo de ejecución en la plataforma de su elección, vea [Microsoft Graph Quick Start](https://developer.microsoft.com/graph/Quick-Start) (Inicio rápido de Microsoft Graph).
- Visite el [repositorio de Microsoft Graph](https://github.com/microsoftgraph) en GitHub para ver todos los ejemplos disponibles de Microsoft Graph. 
